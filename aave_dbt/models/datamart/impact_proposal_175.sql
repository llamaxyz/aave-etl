...
--   left join (select * from aave-prod.datamart.all_revenue where currency = 'native' and token = '0x8dae6cb04688c62d939ed9b68d32bc62e49970b1') n on (
  left join (select * from {{ref('all_revenue')}} where currency = 'native' and token = '0x8dae6cb04688c62d939ed9b68d32bc62e49970b1') n on (
    s.block_day = n.block_day and
    s.market = n.market and
    s.reserve = n.underlying_reserve
  )
--   left join (select * from aave-prod.datamart.all_revenue where currency = 'usd' and token = '0x8dae6cb04688c62d939ed9b68d32bc62e49970b1') u on (
  left join (select * from {{ref('all_revenue')}} where currency = 'usd' and token = '0x8dae6cb04688c62d939ed9b68d32bc62e49970b1') u on (
